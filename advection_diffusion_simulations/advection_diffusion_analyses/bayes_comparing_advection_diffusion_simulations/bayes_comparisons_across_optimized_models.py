...
def plot_model_kde_with_conf_intervals_and_field_data(model_name, list_of_conf_int, number, flexible_airspeeds, ref_pdf):
    with open(model_name) as f:
        d =  json.load(f)
    for key in d:
        pdf = np.asarray(d[key])
    ...
    ax.set_xlabel('windspeed along trajectory, m $\mathregular{s^{-1}}$', fontsize = 12)
    ax.set_ylabel('groundspeed along trajectory, m $\mathregular{s^{-1}}$', fontsize = 12)
    plt.imshow(pdf, cmap='binary', vmin = 0, vmax =0.005, interpolation='nearest', origin='lower', extent = (-2, 3.5, -0.5, 5)) #cmap was 'binary'
    for idx, windspeed in enumerate(windspeeds):
        ...
        prob = pdf[y_index,x_index]
        if isinstance(ref_pdf, (list, tuple, np.ndarray)):
            ref_prob = ref_pdf[y_index,x_index]
            probability = ref_prob/prob
            if probability > 1:
                ax.scatter(windspeed, groundspeeds[idx], color = 'white',edgecolor = 'black', s = 65, zorder = 19)
            if probability < 1:
                ax.scatter(windspeed, groundspeeds[idx], color = 'black', edgecolor = 'black',s=65, zorder = 19)
        else:
            probability = prob
            ax.scatter(windspeed, groundspeeds[idx], color = 'black',zorder = 19)
        probability_string = '% .00E' %(probability)
        ax.text(windspeed+0.03, groundspeeds[idx]+0.03, probability_string,zorder = 20)
    adjust_spines(ax_handle=ax, spines= ['bottom', 'left'])
    if isinstance(ref_pdf, (list, tuple, np.ndarray)):
        ax.text(0.4, -0.3, 'colormap same for all PDFs\ndata points annotated with L1/Lx, with\nx ='+ model_name, fontsize = 12, ha= 'left')
        #######################################################start lil module####
        #adding a subplot that displays bootstrapped distributions of Bayes factors
        resample_number = 40000 #40000
        ...
        for i in range(resample_number):
            ...
            for idx in resampled_indices:
                ...
                y_index = find_nearest_index(y, groundspeeds[idx])
                list_of_log_likelihood_ratios.append(np.log(ref_pdf[y_index, x_index]/pdf[y_index, x_index]))
            ...
            for idx in resampled_indices:
                ...
                list_of_log_likelihood_ratios_outliers_omitted.append(np.log(ref_pdf[y_index, x_index]/pdf[y_index, x_index]))
        ...
        ax2.hist(list_of_log_Bayes_factors_outliers_omitted, bins = binlist, color = 'grey', weights=(10**hist_factor)*np.ones_like(list_of_log_Bayes_factors_outliers_omitted), histtype = 'step')
        binlist = range(int(min(list_of_log_Bayes_factors)), int(max(list_of_log_Bayes_factors))+binwidth, binwidth)
        ax2.hist(list_of_log_Bayes_factors, bins = binlist, color = 'k', weights=(10**hist_factor)*np.ones_like(list_of_log_Bayes_factors), histtype = 'step')
        ax2.set_ylim(0, 175)
        ax2.set_xlim(-600, 600)
        plt.xticks(np.linspace(-600, 600, 3, endpoint = True))
        adjust_spines(ax_handle=ax2, spines= ['bottom'])
        ax2.text(600,45, 'sum of ln(L1/Lx)\ngrey = outliers excluded')
        #######################################################end lil module####
    if flexible_airspeeds:
        ...
        figname = '_model'+str(number)+'_linear_wind_array.png'
    plt.savefig(figname)
    #plt.show()
...
if generate_pdfs_from_wtraj_gtraj_json == 'y':
    ...
    alternate_model_list = ['./model_2/forward_airspeed_1.5_pdf.json',
                        './model_3/airmin_-0.2_airmax_2.0_gpref_1.5_pdf.json',
                        './model_4/forward_airspeed_1.5_pdf.json',
                        './model_5/diff_coeff_10.0_pdf.json',
                        './model_5/diff_coeff_20.0_pdf.json',
                        './model_5/diff_coeff_30.0_pdf.json',
                        './model_5/diff_coeff_40.0_pdf.json',
                        './model_5/diff_coeff_50.0_pdf.json',
                        './model_5/diff_coeff_60.0_pdf.json',
                        './model_5/diff_coeff_70.0_pdf.json',
                        './model_5/diff_coeff_80.0_pdf.json',
                        './model_5/diff_coeff_90.0_pdf.json',
                        './model_5/diff_coeff_100.0_pdf.json',
                        './model_5/diff_coeff_200.0_pdf.json',
                        './model_5/diff_coeff_300.0_pdf.json',
                        './model_5/diff_coeff_400.0_pdf.json',
                        './model_5/diff_coeff_500.0_pdf.json',
                        './model_5/diff_coeff_600.0_pdf.json',
                        './model_5/diff_coeff_700.0_pdf.json',
                        './model_5/diff_coeff_800.0_pdf.json',
                        ]
    reference_model_name = './model_1/airmin_-0.5_airmax_2.0_gpref_1.25_pdf.json'
    with open(reference_model_name) as f:
        ref=  json.load(f)
    for key in ref:
        sideslip_pdf = np.asarray(ref[key])
    print (sideslip_pdf)
    plot_model_kde_with_conf_intervals_and_field_data(model_name=reference_model_name, list_of_conf_int=[], number =1, flexible_airspeeds = False, ref_pdf = False)
    for number, model in enumerate(alternate_model_list):
        plot_model_kde_with_conf_intervals_and_field_data(model_name=model, list_of_conf_int=[0.95], number = number +2, flexible_airspeeds = False, ref_pdf = sideslip_pdf)
    ...
    for number, model in enumerate(alternate_model_list):
        ...
        for key in alt_model:
            alternate_pdf = np.asarray(alt_model[key])
        resample_number = 30000 #40000
        list_of_log_Bayes_factors = np.zeros(resample_number)
        list_of_log_Bayes_factors_outliers_omitted = np.zeros(resample_number)
        for i in range(resample_number):
            resampled_indices = np.random.choice(len(groundspeeds),len(groundspeeds), replace = True)
        ...
        ax = plt.subplot(18,1,number+1)
        hist_factor = -2.
        binwidth = 30

        binlist = range(int(min(list_of_log_Bayes_factors_outliers_omitted)), int(max(list_of_log_Bayes_factors_outliers_omitted))+binwidth, binwidth)
        ax.hist(list_of_log_Bayes_factors_outliers_omitted, bins = binlist, color = 'grey', weights=(10**hist_factor)*np.ones_like(list_of_log_Bayes_factors_outliers_omitted), histtype = 'step')
