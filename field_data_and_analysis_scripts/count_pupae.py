...
def get_contours_from_frame(image, adaptive_thresh_neighborhood, adaptive_thresh_offset):
    ...
    cv2.imshow('press any key for next image; "q" to quit',thresh_img_smooth)
...
def count_flies(image,
    ...
    cv2.imshow('press any key for next image; "q" to quit',image)
    wait_key_val = cv2.waitKey(0) & 0xFF
    if wait_key_val == ord('q'):
        return "quit"
    else:
...
#def calculate_pupal_mode(pupal_histogram):
...
#pupal_mode_size = calculate_pupal_mode(pupal_histogram) # <------- working on this
