...
if generate_pdfs_from_wtraj_gtraj_json == 'y':
    ...
    plot_model_kde_with_conf_intervals_and_field_data(model_name=reference_model_name, list_of_conf_int=[], number =1, flexible_airspeeds = False)
    for number, model in enumerate(alternate_model_list):
        plot_model_kde_with_conf_intervals_and_field_data(model_name=model, list_of_conf_int=[], number = number +2, flexible_airspeeds = False)
    ...
    for number, model in enumerate(alternate_model_list):
        ...
        print ('')
    figname = 'log_bayes_factor_distributions_outliers.svg'
    plt.savefig(figname)
    # plt.show()
