...
"""
allow_flexible_airspeeds = raw_input("allow flexible airspeeds, or do flies drop out if they can't hold their set trajectory? (True/False)")
...
def calculate_heading_vector (wind_speed, wind_dir,
        ...
        if allow_flexible_airspeeds == True:
            if perp_mag > forward_airspeed_limit:
                print ('perp_mag exceeds forward airspeed limit') #immediately excludes some cases, where the fly is just not going to be able to maintain this trajectory.
                return(None, None)
                print ('testing whether we get to this line?')
                ...
                if np.sign(scalar_projection(wind_vector, fly_trajectory_unit_vector)) == -1: # wind opposing intended traj
                    #fly would be pushed backwards by the wind; this triggers a simple switch into "high gear," that is a forward_airspeed of the absolute limit.
                    forward_airspeed = forward_airspeed_limit
                    print ('forward airspeed set to ' +str(forward_airspeed))
...
if allow_flexible_airspeeds == True:
    with open('./model_4_flies_kick_into_1.8ms.json', mode = 'w') as f:
        json.dump(dictionary,f, indent = 1)
else:
    with open('./model_4_no_airspeed_flexibility.json', mode = 'w') as f:
        json.dump(dictionary,f, indent = 1)
