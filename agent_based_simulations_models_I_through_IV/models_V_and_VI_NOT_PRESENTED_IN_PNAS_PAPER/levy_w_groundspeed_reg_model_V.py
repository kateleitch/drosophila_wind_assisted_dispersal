...
def run_simulation(mu, wind_direction, wind_speed, default_airspeed, s_angles, s_pdf, min_run_length = 2.0, max_run_length = 50.0, fly_number = 20):
    ...
    figname = './groundspeed_reg/mu_' +str(mu)+'windspeed_'+str(wind_speed)+'_min_run_'+str(min_run_length)+'_gsreg.png'
    plt.savefig(figname)
...
#
# with open('./groundspeed_reg/gtraj_wtraj.json', mode = 'w') as f:
...
for wind_speed in wind_speed_list:
    ...
    plt.close()
with open('./groundspeed_reg/w_traj_g_traj_ending_angles.json', mode = 'w') as f:
