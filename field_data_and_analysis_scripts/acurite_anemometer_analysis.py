...
"""
winddirection transformation is based on the anemometer's "lower notch" oriented north, which has always been my convention in the field.
any time "wind direction" is mentioned, this means "direction the wind is coming FROM"
the anemometer's output wraps when the wind is coming from the south
"""

"""
this script will read in acurite .csv files and produce a comparable plot of wind vectors as the OG script "anemometer_analysis.py"
"""
class AcuriteAnemometerAnalyzer:
    def __init__(self, directory, ax_handle, time_shift, annie_sim_data = False, turn_off_text = False, turn_off_scalebar = False, orient_circmean_from_north = False, turn_off_title = False, bin_duration = 60., declare_fixed_scale = 0, turn_off_wind = False, plot_vectors_as_function_of_time = False, savefig = True):
    ...
    def get_time_since_release_from_acurite_timestamp (self, timestamp):
        ...
        if timestamp.split(' ')[2] == 'PM':
            #t24h = str(int(t.split(':')[0])+12)+':'+t.split(':')[1:]
            h24 = str(int(t.split(':')[0]) +12)
            min = t.split(':')[1]
            sec =t.split(':')[2]
            t24h = h24+':'+min+':'+sec
        else:
            t24h = t
        timestamp_seconds = int(t24h.split(':')[0])*3600 +   int(t24h.split(':')[1])*60 + float(t24h.split(':')[2])
        time_elapsed = timestamp_seconds - release_time_seconds
        return time_elapsed
    ...
    def run(self):
        wind_filename_list = self.get_filenames(self.directory+'/weather_data/', contains = 'acuriteweather_trimmed', does_not_contain=['.ods'])
        ...
        with open(wind_file_name, 'r') as wind_direction_file:
            for line in wind_direction_file:
                ...
                if line_counter == 1:
                    continue
                winddirection_list.append(float(line.split(',')[11][1:-1])-self.mag_declin_deg) # in degrees from TRUE north
                timestamp_list.append((line.split(',')[0][1:-1])) #
                windspeed_list.append(float(line.split(',')[8][1:-1])) # in meters per second
        sec_since_release_list =[]
        for timestamp in timestamp_list:
            sec_since_release_list.append(self.get_time_since_release_from_acurite_timestamp(timestamp))
        plot_how_many_minutes_pre_release = 0   # < --------- NEEDS TO BE MADE FLEXIBLE
        plot_how_many_minutes_post_release = 20 # < --------- NEEDS TO BE MADE FLEXIBLE
        indices_to_plot=[]
        ...
        bin_duration = self.bin_duration # always specify in seconds
        binned_wind_dir = []
        binned_windspeeds = []
        ...
        print ([s/60.0 for s in sec_since_release_sublist])
        ...
        if self.plot_vectors_as_fnc_of_time == False:
            ...
            ymax = 5.0
            ymin = -0.5
            xmax = np.max(time_bin_list)*1.05
            xmin = -160
            column_num =1.0
            y_scale = 1
            ...
            ax.plot(time_bin_list, binned_windspeeds, 'gray')
        ...
        if not self.turn_off_text:
            ...
            vec_avg_speed_mph = vector_sum_speed/number_of_vectors_summed  # last index can contain NaN
            vec_avg_speed_mps = vec_avg_speed_mph/2.23694

            unit_vector_sum = np.array([0,0])
            vec_count = 0
            wind_vector_points_trimmed = wind_vector_points[1:vector_index]
            previous_point = np.array([0,0])
            for current_point in wind_vector_points_trimmed:
                q = current_point - previous_point
                current_unit_vector = q / float(np.linalg.norm(q))
                print (np.arctan2(current_unit_vector[1], current_unit_vector[0])*180/np.pi)
                unit_vector_sum = unit_vector_sum + current_unit_vector
                vec_count +=1
                previous_point = current_point
                print ('running vec strength: ' + str(np.linalg.norm(unit_vector_sum)/float(vec_count)))
                print()
            print ('vec count: ' +str(vec_count))
            vector_strength = np.linalg.norm(unit_vector_sum)/float(vec_count)
            print ('wind vector strength (ranges from 0 to 1): '+ str(vector_strength))

            print ('vector averaged speed, meters per second: ' +str(vec_avg_speed_mps))
            ...
            ax.text(scalebar_position[0]+scalebar_mph/2., scalebar_position[1] - y_range/4,
                    'avg wind ' +('%.1f'%vec_avg_speed_mph)+ ' mph \n' + 'out of ' + compass_point, ha = 'center')
        ...
        return degrees_to_rotate, mean_dir, vec_avg_speed_mps
######### run here
# fig = plt.figure()
# ax = plt.subplot(111)
#
# dir = './2017_04_30'
# # dir = './2017_04_15'
# anemometer_analyzer = AcuriteAnemometerAnalyzer(directory = dir,
#                                                             ax_handle = ax,
#                                                             time_shift = 0,
#                                                             turn_off_scalebar = False,
#                                                             orient_circmean_from_north = False,
#                                                             turn_off_title = True,
#                                                             turn_off_text = False,
#                                                             bin_duration = 60.,
#                                                             declare_fixed_scale = 0,
#                                                             plot_vectors_as_function_of_time = False,
#                                                             savefig = False)
# degrees_to_rotate, mean_dir, avg_speed_mps = anemometer_analyzer.run()
# plt.show()
