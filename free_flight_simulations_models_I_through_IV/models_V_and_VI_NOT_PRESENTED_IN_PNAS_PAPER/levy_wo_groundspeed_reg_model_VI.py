...
def calculate_trajectory_vector_w_groundspeed_reg (wind_speed, wind_dir,
...
def calculate_trajectory_vector (wind_speed, wind_dir,
        ...
        if scalar_projection(trajectory_vector, fly_heading_unit_vector) < 0: #fly would be pushed backwards along her body axis
            ...
            if np.linalg.norm(airspeed_vector_along_body_axis) > forward_airspeed_limit:
                print ('maintaining this heading is not possible in this wind')
                return(None,None)
        return trajectory_vector

def saccade_angle_pdf_from_muijres(x):
    prob_distribution = normal(x, mean = 93, sigma = 27)
    return prob_distribution/np.sum(prob_distribution)

def run_simulation(mu, wind_direction, wind_speed, default_airspeed, s_angles, s_pdf, min_run_length = 2.0, max_run_length = 50.0, fly_number = 20):
    ...
    for fly in range(fly_number):
        ...
        heading_ang = np.random.choice(np.linspace(0,2*np.pi,360,endpoint = False), 1)[0] #initializing
        ...
        while np.linalg.norm(position) < trap_radius:
            ang = np.random.choice(s_angles, 1, p = s_pdf)[0]*(np.pi/180.)
            saccade_angle_list.append(ang)
            heading_ang += ang

            trajectory_vector = calculate_trajectory_vector (wind_speed = wind_speed, wind_dir= wind_direction,
                                        fly_heading = heading_ang, forward_airspeed = 1.0) #trajectory_vector in m/s
    ...
    #histogram = ax.hist([p/60. for p in total_flight_duration_list], histtype = 'step', color = 'black')
    ...
    figname = './airspeed_fixed_1.0/mu_' +str(mu)+'windspeed_'+str(wind_speed)+'_min_run_'+str(min_run_length)+'_gsreg.png'
    plt.savefig(figname)
    return np.array(total_flight_duration_list), histogram, groundspeeds_along_trajectory, windspeeds_along_trajectory, ending_angular_position_list
...
#############################################333333
#mu_list = [2.0]#[1.5, 2.0, 2.5]
mu = 2.0
#min_run_length_list = [1.0]#[1.0, 10.0, 100.0] #note: Michael and I discussed [0.1, 1.0, 10.0] but 0.1 takes forever
min_run_length = 1.0
max_run_length = 1000.0 # very generous
# theta_range_width_list = [90]#[45, 90, 180]
s_angles = np.linspace(0,180,181,endpoint = True)
s_pdf = saccade_angle_pdf_from_muijres(x =s_angles)
...
#wind_speed_list = np.linspace(0.0, 2.6, 14, endpoint = True) # meters per second
...
#     flight_duration_list, histogram, groundspeeds_along_trajectory, windspeeds_along_trajectory= run_simulation(mu, wind_direction, wind_speed, default_airspeed, s_angles, s_pdf, min_run_length, max_run_length, fly_number)
#     dict[wind_speed] = {'g_traj': groundspeeds_along_trajectory, 'w_traj': windspeeds_along_trajectory}
#
# with open('./airspeed_fixed_1.0/gtraj_wtraj.json', mode = 'w') as f:
#     json.dump(dict,f, indent = 1)


dict_edited = {}
for wind_speed in wind_speed_list:
    flight_duration_list, histogram, groundspeeds_along_trajectory, windspeeds_along_trajectory, ending_angular_position_list= run_simulation(mu, wind_direction, wind_speed, default_airspeed, s_angles, s_pdf, min_run_length, max_run_length, fly_number)

    dict_edited[wind_speed] = {'g_traj': groundspeeds_along_trajectory, 'w_traj': windspeeds_along_trajectory, 'angular_positions': list(ending_angular_position_list)}
    plt.close()
with open('./airspeed_fixed_1.0/w_traj_g_traj_ending_angles.json', mode = 'w') as f:
    json.dump(dict_edited,f, indent = 1)
