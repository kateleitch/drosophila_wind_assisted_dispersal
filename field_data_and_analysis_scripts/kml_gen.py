...
class kmlGenerator:
    def __init__(self,dir):
        ...
        self.datestring = now.strftime("%Y_%m_%d_")
        self.dir = dir

    def getCirclecoordinates(self,lat, lon, radius, starting_angle, angle, last_iteration, aslist=False):
    ...
    def run(self):
        ...
        self.datestring = now.strftime("%Y_%d_%m_")
        print ('')
        ...
        for i in range(release_site_number):
            ...
            trap_prefix = self.datestring # for name of pin
            filename = self.dir+"/trap_coords.kml"
            ...
            for index, radius in enumerate(radius_list):
                ...
                coordinatesList_path += ' '+self.getCirclecoordinates(lat, lon, radius, starting_angle, intertrap_angle, last_iteration, aslist=False);
                coordinatesList_point += self.getCirclecoordinates(lat, lon, radius, starting_angle, intertrap_angle, last_iteration, aslist=True);
                point_list_for_json += self.getCirclecoordinates(lat, lon, radius, starting_angle, intertrap_angle, last_iteration, aslist=True);
        ...
        for i, coordinate in enumerate(coordinatesList_point):
          kml += "  <Placemark>\n"
          kml += "    <name>" + self.datestring +trap_naming_list[i] + "</name>\n"
        ...
        with open(self.dir+"/trap_coords.json", mode='w') as g:
